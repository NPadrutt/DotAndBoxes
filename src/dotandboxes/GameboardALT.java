 ...
 * @author Caro
 */
public class GameboardALT extends JPanel{
     ...
     * @param columns 
     */
    public GameboardALT(int rows, int columns) {
        super();
        this.rows = rows;
        ...
        for (int y = 0; y <= (2*columns); y++) {
            for (int x = 0; x <= (2*rows); x++) {
                ...
                if ((y%2 == 0) && (x%2 == 0)) {
                    ...
                    this.add(new DotPictureALT(), gridBag);
                }
                else if ((y%2 == 0) && (x%2 != 0)) {
                    gridBag.fill = GridBagConstraints.HORIZONTAL;
                    this.add(new LinePictureALT(LinePicture.HORIZONTAL), gridBag);
                }
                else if ((y%2 != 0) && (x%2 == 0)) {
                    gridBag.fill = GridBagConstraints.VERTICAL;
                    this.add(new LinePictureALT(LinePicture.VERTICAL), gridBag);
                }
                else {
                    gridBag.fill = GridBagConstraints.BOTH;
                    this.add(new BoxPictureALT(), gridBag);
                }
            }
        }
    }
    ...
    public static void main(String[] args) {
        ...
        GameboardALT game = new GameboardALT(10,10);
        frame.add(game);
